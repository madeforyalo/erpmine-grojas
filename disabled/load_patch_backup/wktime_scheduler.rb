	...
	Rails.application.config.after_initialize do
		if ActiveRecord::Base.connection.table_exists? "#{Setting.table_name}"
			if ActiveRecord::Base.connection.table_exists?("#{WkNotification.table_name}") && WkNotification.notify('nonSubmission')
				if (!Setting.plugin_redmine_wktime_lite['wktime_use_approval_system'].blank? && Setting.plugin_redmine_wktime_lite['wktime_use_approval_system'].to_i == 1)
					submissionDeadline = Setting.plugin_redmine_wktime_lite['wktime_submission_deadline']
					hr = Setting.plugin_redmine_wktime_lite['wktime_nonsub_sch_hr']
					min = Setting.plugin_redmine_wktime_lite['wktime_nonsub_sch_min']
			...
			if (!Setting.plugin_redmine_wktime_lite['wktime_period_end_process'].blank? && Setting.plugin_redmine_wktime_lite['wktime_period_end_process'].to_i == 1)
			...
			if (!Setting.plugin_redmine_wktime_lite['wktime_auto_import'].blank? && Setting.plugin_redmine_wktime_lite['wktime_auto_import'].to_i == 1)
				...
				importScheduler.every intervalMin do
					begin
						...
						filePath = Setting.plugin_redmine_wktime_lite['wktime_file_to_import']
			...
			if (!Setting.plugin_redmine_wktime_lite['wktime_auto_generate_salary'].blank? && Setting.plugin_redmine_wktime_lite['wktime_auto_generate_salary'].to_i == 1)
			...
			if (!Setting.plugin_redmine_wktime_lite['wktime_auto_generate_invoice'].blank? && Setting.plugin_redmine_wktime_lite['wktime_auto_generate_invoice'].to_i == 1)
				invoiceScheduler = Rufus::Scheduler.new
				invPeriod = Setting.plugin_redmine_wktime_lite['wktime_generate_invoice_period']
				invDay = Setting.plugin_redmine_wktime_lite['wktime_generate_invoice_day']
				genInvFrom = Setting.plugin_redmine_wktime_lite['wktime_generate_invoice_from'].to_date
			...
			if (!Setting.plugin_redmine_wktime_lite['auto_apply_depreciation'].blank? && Setting.plugin_redmine_wktime_lite['auto_apply_depreciation'].to_i == 1)
				...
				depreciationFreq = wkinventory_helper.getFrequencyMonth(Setting.plugin_redmine_wktime_lite['wktime_depreciation_frequency'])
				#Scheduler will run at 12:01 AM on 1st of every month
				cronSt = "01 00 01 * *"
			...
			if (!Setting.plugin_redmine_wktime_lite['wk_auto_shift_scheduling'].blank? && Setting.plugin_redmine_wktime_lite['wk_auto_shift_scheduling'].to_i == 1)
			...
			if (Setting.plugin_redmine_wktime_lite['activity_remainder_mail'].present? && Setting.plugin_redmine_wktime_lite['activity_remainder_mail'].to_i == 1)
